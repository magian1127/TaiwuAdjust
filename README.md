...
| **突破界面疗伤按钮** | ✅ 开 | 实际突破（走格子）界面右上角加「疗伤」按钮：有伤可治时亮色可点（点击即治一次，大夫=病患=主角），无伤时灰色不可点。 |
| **调试模式** | ❌ 关 | 开启后输出详细运行日志到 `Player.log`，用于排查问题。 |

四个功能都可在游戏内 **MOD 管理器 → 综合调整 → 设置** 里独立开关，改完即时生效，无需重启。
...
- [docs/cases/突破界面疗伤按钮.md](../docs/cases/突破界面疗伤按钮.md) — 动态创建按钮（TMP 字体复制）+ 疗伤异步 API
